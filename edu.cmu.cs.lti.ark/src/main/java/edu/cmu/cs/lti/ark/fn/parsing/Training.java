...
public class Training {
	...
	public void init(final String modelFile, final String alphabetFile,
			final ArrayList<FrameFeatures> list, final String lexiconFile,
			final String frFile, final String binaryFactorPresent) {
	...
	public void init(final String modelFile, final String alphabetFile,
			final ArrayList<FrameFeatures> list, final String frFile,
			final String binaryFactorPresent, final String reg,
			final double lambda, final int numThreads) {
		mModelFile = modelFile;
		mAlphabetFile = alphabetFile;
		initModel();
		mFrameList = list;
	...
	public double getValueOfSample(final int index) {
		return 0.0;
	}

	public Pair<Double, double[]> getDerivativesOfSample(
			final double[] sumDers, final int index) {
	...
	public void trainSGA(final int TOTAL_PASSES, final int batchsize) {
		int sizeOfData = mFrameList.size();
		int maxUpdates = (int) (((double) TOTAL_PASSES * (double) sizeOfData) / batchsize);
		int totalUpdates = 0;
		int countPasses = 0;
		...
		while (totalUpdates < maxUpdates) {
			...
			for (int j = 0; j < arr.length; j++) {
				...
				Pair<Double, double[]> p = getDerivativesOfSample(sumDers,
						sampleIndex);
				sumDers = p.getSecond();
			}
			...
			if (countDataEncountered >= sizeOfData) {
				System.out.println("\nCompleted pass number:"
						+ (countPasses + 1) + " total updates till now:"
						+ totalUpdates);
	...
	public void processBatch(final int taskID, final int start, int end) {
		int threadID = taskID % mNumThreads;
		// System.out.println("Processing batch:" + taskID + " thread ID:" +
		// threadID);
	...
	public void runCustomLBFGS() throws Exception {
		...
		do {
			...
			LBFGS.lbfgs(modelSize, FNConstants.m_num_corrections, W, m_value,
					mGradients, false, // true if we're providing the diag of
										// cov matrix Hk0 (?)
					diagco, // the cov matrix
					iprint, // type of output generated
					FNConstants.m_eps, FNConstants.xtol, // estimate of machine
															// precision
	...
	public int[] getRandArray(final int batchSize, final int sizeOfData) {
		int count = 0;
	...
	public void writeModel(final String modelFile) {
