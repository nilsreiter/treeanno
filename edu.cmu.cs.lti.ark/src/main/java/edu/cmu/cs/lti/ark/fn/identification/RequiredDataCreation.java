...
public class RequiredDataCreation {
	...
	public static void main(final String[] args) {
		...
		Pair<Map<String, Set<String>>, Map<String, THashMap<String, Set<String>>>> pair = buildHVWordNetCache(options);
		Map<String, Map<String, Set<String>>> revisedMap = reviseRelMap(
				pair.getSecond(), options);
		String fmFile = options.frameNetMapFile.get();
		THashMap<String, THashSet<String>> frameMap = (THashMap<String, THashSet<String>>) SerializedObjects
				.readSerializedObject(fmFile);
		RequiredDataForFrameIdentification req = new RequiredDataForFrameIdentification(
				relWords, pair.getFirst(), pair.getSecond(), frameMap, cMap,
				revisedMap, hvLemmas);
		SerializedObjects.writeSerializedObject(req,
				options.fnIdReqDataFile.get());
	}

	public static Map<String, String> getHVLemmas(final FNModelOptions options) {
		String fmFile = options.frameNetMapFile.get();
		...
		for (String frame : keySet) {
			...
			for (String hUnit : hus) {
				...
				for (int i = 0; i < hiddenToks.length; i++) {
					...
					String lowerCaseLemma = wnr.getLemmaForWord(arr[0], arr[1])
							.toLowerCase();
					lemmaMap.put(arr[0] + "_" + arr[1], lowerCaseLemma);
					hiddenUnitLemmas += wnr.getLemmaForWord(arr[0], arr[1])
							.toLowerCase() + " ";
				}
		...
		SerializedObjects.writeSerializedObject(lemmaMap,
				options.lemmaCacheFile.get());
		return lemmaMap;

	}

	public static Map<String, Map<String, Set<String>>> reviseRelMap(
			final Map<String, THashMap<String, Set<String>>> relMap,
			final FNModelOptions options) {
		Map<String, Map<String, Set<String>>> revisedMap = new HashMap<String, Map<String, Set<String>>>();
	...
	public static Pair<Map<String, Set<String>>, Map<String, THashMap<String, Set<String>>>> buildHVWordNetCache(
			final FNModelOptions options) {
		String fmFile = options.frameNetMapFile.get();
		String wnConfigFile = options.wnConfigFile.get();
		String stopFile = options.stopWordsFile.get();
		...
		Map<String, Set<String>> relatedWordsForWord = wnr
				.getRelatedWordsForWord();
		Map<String, THashMap<String, Set<String>>> wordNetMap = wnr
				.getWordNetMap();
		...
		return new Pair<Map<String, Set<String>>, Map<String, THashMap<String, Set<String>>>>(
				relatedWordsForWord, wordNetMap);
	}

	public static THashMap<String, THashSet<String>> getHVCorrespondence(
			final FNModelOptions options) {
		String fmFile = options.frameNetMapFile.get();
		String wnConfigFile = options.wnConfigFile.get();
		String stopFile = options.stopWordsFile.get();
	...
	public static THashSet<String> getRelatedWords(final FNModelOptions options) {
		String fmFile = options.frameNetMapFile.get();
		String wnConfigFile = options.wnConfigFile.get();
		String stopFile = options.stopWordsFile.get();
		String luXmlDir = options.luXmlDir.get();
		THashMap<String, THashSet<String>> mFrameMap = (THashMap<String, THashSet<String>>) SerializedObjects
				.readSerializedObject(fmFile);
		WordNetRelations wnr = new WordNetRelations(stopFile, wnConfigFile);
	...
	public static THashSet<String> getListOfLUs(final String directory,
		...
		for (String file : files) {
			...
			if (corpora == null || corpora.length == 0) {
				...
				String lemma = wnr
						.getLemmaForWord(word, conversionMap.get(pos));
	...
	public static THashSet<String> getAllRelatedWords(
			final THashMap<String, THashSet<String>> mFrameMap,
			final WordNetRelations mWNR) {
		THashSet<String> result = new THashSet<String>();
		...
		for (String string : set) {
			...
			for (String hu : hus) {
				...
				if (wps.length == 1) {
					...
					if (p.startsWith("N")
							|| /* p.startsWith("JJ")|| */p.startsWith("V")) {
						THashMap<String, Set<String>> rlMap = mWNR
								.getAllRelationsMap(l);
	...
	public static Set<String> getRevisedSet(final Set<String> rl,
			final boolean check) {
		THashSet<String> result = new THashSet<String>();
		for (String r : rl) {
			r = r.toLowerCase();
	...
	public static Set<String> getSelectedRelatedWords(
			final THashMap<String, Set<String>> rlMap) {
		Set<String> keys = rlMap.keySet();
		THashSet<String> result = new THashSet<String>();
		for (String key : keys) {
			if (key.equals("identity")/*
									 * ||key.equals("synonym")||key.equals(
									 * "derived-form")||key.equals("morph")
									 */)
