...
public class ParserDriver {
	...
	/*
	 * required flags: mstmode mstserver mstport posfile test-parsefile
	 * stopwords-file wordnet-configfile fnidreqdatafile goldsegfile userelaxed
	 * testtokenizedfile idmodelfile alphabetfile framenet-femapfile eventsfile
	 * spansfile model useGraph frameelementsoutputfile alllemmatagsfile
	 */
	...
	public static void main(final String[] args) {
		...
		try {
			...
			System.err.println("Could not open POS tagged file: " + posFile
					+ ". Exiting.");
			System.exit(-1);
		}
		try {
			...
			System.err.println("Could not open tokenized file: "
					+ tokenizedFile + ". Exiting.");
			System.exit(-1);
		}
		...
		if (posReader != null) {
			try {
				...
				System.err
						.println("Could not close POS input stream. Exiting.");
				System.exit(-1);
			}
		}
		if (tokenizedReader != null) {
			try {
				...
				System.err
						.println("Could not close tokenizedReader input stream. Exiting.");
				System.exit(-1);
			}
		}
	}

	private static void runParser(final BufferedReader posReader,
			final BufferedReader tokenizedReader, final WordNetRelations wnr,
			final FNModelOptions options, final String serverName,
		...
		Map<String, THashMap<String, Set<String>>> wordNetMap = r
				.getWordNetMap();
		...
		System.err.println("idParamFile: " + options.idParamFile.get());
		TObjectDoubleHashMap<String> paramList = FrameIdentificationRelease
				.parseParamFile(options.idParamFile.get());
		System.out.println("Initializing frame identification model...");
		FastFrameIdentifier idModel = new FastFrameIdentifier(paramList, "reg",
				0.0, frameMap, null, cMap, relatedWordsForWord,
				revisedRelationsMap, hvLemmas);
		boolean usegraph = !options.useGraph.get().equals("null");
		SmoothedGraph sg = null;
		if (usegraph) {
			sg = (SmoothedGraph) SerializedObjects
					.readSerializedObject(options.useGraph.get());
			System.out.println("Read graph successfully from: "
					+ options.useGraph.get());
		}
		...
		CreateAlphabet.setDataFileNames(options.alphabetFile.get(),
				...
				options.eventsFile.get(), options.spansFile.get());
		Decoding decoding = new Decoding();
		decoding.init(options.modelFile.get(), options.alphabetFile.get());
		...
		if (goldSegFile == null || goldSegFile.equals("null")
				|| goldSegFile.equals("")) {
			...
			try {
				...
				System.err.println("Could not open gold segmentation file:"
						+ goldSegFile);
				System.exit(-1);
			}
		}
		...
		try {
			bWriter1 = new BufferedWriter(
					new FileWriter(allLemmaTagsOutputFile));
		} catch (IOException e) {
			System.err.println("Could not open file to write: "
					+ allLemmaTagsOutputFile);
		}
		...
		try {
			...
			if (serverName == null) {
				parseReader = new BufferedReader(new FileReader(
						options.testParseFile.get()));
			}
			do {
				int index = 0;
				posLines.clear();
				...
				parseSets.clear();
				System.out.println("Processing batch of size:" + BATCH_SIZE
						+ " starting from: " + count);
				...
				if (serverName != null) {
					parseSets = getParsesFromServer(serverName, serverPort,
							posLines);
				}
				ArrayList<String> allLemmaTagsSentences = getAllLemmaTagsSentences(
						tokenizedLines, parseSets, wnr);
				...
				if (segmentationMode == 0) {
					...
					segs = seg.findSegmentationForTest(tokenNums,
							allLemmaTagsSentences, allRelatedWords);
				...
				inputForFrameId = ParseUtils
						.getRightInputForFrameIdentification(segs);

				// 2. frame identification
				for (String input : inputForFrameId) {
					String[] toks = input.split("\t");
					int sentNum = new Integer(toks[2]); // offset of the
														// sentence within the
														// loaded data (relative
														// to
														// options.startIndex)
					String bestFrame = null;
					if (sg == null) {
						bestFrame = idModel.getBestFrame(input,
								allLemmaTagsSentences.get(sentNum));
					} else {
						bestFrame = idModel.getBestFrame(input,
								allLemmaTagsSentences.get(sentNum), sg);
					}
					String tokenRepresentation = FrameIdentificationRelease
							.getTokenRepresentation(toks[1],
									allLemmaTagsSentences.get(sentNum));
					String[] split = tokenRepresentation.trim().split("\t");
					idResult.add(1 + "\t" + bestFrame + "\t" + split[0] + "\t"
							+ toks[1] + "\t" + split[1] + "\t" + sentNum); // BestFrame\tTargetTokenNum(s)\tSentenceOffset
				}
				// 3. argument identification
				CreateAlphabet.run(false, allLemmaTagsSentences, idResult, wnr);
				LocalFeatureReading lfr = new LocalFeatureReading(
						options.eventsFile.get(), options.spansFile.get(),
						idResult);
				try {
					...
					System.err
							.println("Could not read local features. Exiting.");
					System.exit(-1);
				}
	...
	private static ArrayList<String> getAllLemmaTagsSentences(
			final ArrayList<String> tokenizedLines,
			final ArrayList<ArrayList<String>> parses,
			final WordNetRelations wnr) {
		ArrayList<String> neSentences = AllAnnotationsMergingWithoutNE
				.findDummyNESentences(tokenizedLines);
		ArrayList<String> perSentenceParses = OneLineDataCreation
				.getPerSentenceParses(parses, tokenizedLines, neSentences);
		...
		for (String line : perSentenceParses) {
			...
			for (int i = 0; i < sentLen; i++) {
				String lemma = wnr.getLemmaForWord(toks[i + 1].toLowerCase(),
						toks[i + 1 + sentLen]);
				outLine += lemma + "\t";
			}
	...
	public static ArrayList<ArrayList<String>> getParsesFromServer(
			final String server, final int port,
			final ArrayList<String> posLines) {
		...
		try {
			...
			System.err
					.println("Could not close input channel from server. Exiting.");
			System.exit(-1);
		}
		return ret;
	}

	public static ArrayList<String> readCoNLLParse(final BufferedReader bReader) {
