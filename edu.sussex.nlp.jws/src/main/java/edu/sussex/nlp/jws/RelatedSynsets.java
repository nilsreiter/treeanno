...
public class RelatedSynsets {
	...
	// 'Utility'
	// 1. just get the *all* the synset (IDs) that are connected to a
	// word/synset (i.e. no Pointer types, just the IDs)
	// That is, we get both semantic *and* lexical synsets ( we have to use the
	// Word objects in the synset to get the lexical synsets)
	// i.e. we sort of have to go round the back door
	// vers. 1 for a "word"
	public HashSet<ISynsetID> getAllRelatedSynsetsNoTypes(String wordIN,
			int sense, String pos) {
		HashSet<ISynsetID> set = new HashSet<ISynsetID>();
		set.addAll((getAllRelatedSynsetsAndTheirTypes(wordIN, sense, pos))
				.keySet());
		return (set);
	}
	...
	// 2. get the synsets (IDs) and their Pointer type - gets *both* types of
	// related synsets 1. semantic e.g. <hypernyms> and 2. lexical e.g. antonyms
	// vers. 1 for a "word"
	public Hashtable<ISynsetID, IPointer> getAllRelatedSynsetsAndTheirTypes(
			String wordIN, int sense, String pos) {
		Hashtable<ISynsetID, IPointer> set = new Hashtable<ISynsetID, IPointer>();
		IIndexWord idxWord = null;
		if (pos.equalsIgnoreCase("n")) {
			idxWord = dict.getIndexWord(wordIN, POS.NOUN);
		}
		if (pos.equalsIgnoreCase("v")) {
			idxWord = dict.getIndexWord(wordIN, POS.VERB);
		}
		if (pos.equalsIgnoreCase("a")) {
			idxWord = dict.getIndexWord(wordIN, POS.ADJECTIVE);
		}
		if (pos.equalsIgnoreCase("r")) {
			idxWord = dict.getIndexWord(wordIN, POS.ADVERB);
		}
		if (idxWord == null)
			return (set);
		// ............................................................................................................
		...
		// 1. for {synsets}
		Map<IPointer, List<ISynsetID>> semantic_synset_pointers = synset
				.getRelatedMap();
		...
		Map<IPointer, List<IWordID>> semantic_word_pointers = word
				.getRelatedMap();
	...
	// vers. 2 for a {synset}
	// gets *both* types of related synsets 1. semantic e.g. <hypernyms> and 2.
	// lexical e.g. antonyms
	public Hashtable<ISynsetID, IPointer> getAllRelatedSynsetsAndTheirTypes(
			ISynset synset) {
		Hashtable<ISynsetID, IPointer> set = new Hashtable<ISynsetID, IPointer>();
		// 1. for {synsets}
		...
		// 2. for Word objects (in the synset)
		List<IWord> words = synset.getWords(); // get the Word objects in the
												// 'synset'
	...
	public static void main(String[] args) throws IOException {
		...
		ISynset synset = word.getSynset();
		// 1. for a "word"
		// Hashtable<ISynsetID, IPointer> set1 =
		// ap.getAllRelatedSynsetsAndTheirTypes("sweet", 1, "a");
		// 2. for a {synset}
		Hashtable<ISynsetID, IPointer> set2 = ap
				.getAllRelatedSynsetsAndTheirTypes(synset);
	}
}
