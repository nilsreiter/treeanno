...
public class FrameElementRelation implements IHasID, Serializable, IHasName {
    ...
    protected FrameElementRelation(final FrameRelation relation,
	    final Element xmlnode) throws FrameElementNotFoundException {
	frameRelation = relation;
	id = xmlnode.attributeValue("ID");
	superFrameElement =
		frameRelation.superFrame.getFrameElement(xmlnode
			.attributeValue("superFEName"));
	subFrameElement =
		frameRelation.subFrame.getFrameElement(xmlnode
	...
	superFrameElement.getRelationsAsSuper()
		.get(frameRelation.getFrameNetRelation())
		.put(frameRelation.getSubFrame(), this);
	subFrameElement.getRelationsAsSub()
		.get(frameRelation.getFrameNetRelation())
		.put(frameRelation.getSuperFrame(), this);
