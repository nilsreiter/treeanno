 ...
 * 
 */
public class FNLexicalUnit13 extends FNLexicalUnit {
    /**
	 * 
	 */
     ...
     */
    protected FNLexicalUnit13(final FNDatabaseReader reader, final Frame frame,
	    final Element node) {
	this.frame = frame;
	...
	List<?> lexemeNodelist = node.element("lexemes").elements("lexeme"); // xpath.selectNodes(node);
	for (int i = 0; i < lexemeNodelist.size(); i++) {
	    Element lexemeElement = (Element) lexemeNodelist.get(i);
	    Lexeme lexeme = new FNLexeme13(lexemeElement);
	    lexemes.add(lexeme);
	}
