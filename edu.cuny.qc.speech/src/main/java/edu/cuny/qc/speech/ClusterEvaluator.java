...
public class ClusterEvaluator {
	...
	/**
	 * Calculates the NVI measure (Reichart & Rappoport, 2009)
	 * 
	 * @author Nils Reiter
	 * @return
	 */
	public double getNVI() {
		double h_c = 0.0;
		double h_k = 0.0;
		Double[] class_v = data.getColSum();
		Double[] clust_v = data.getRowSum();
		...
		for (int i = 0; i < data.getNumCols(); i++) {
			h_c -= class_v[i] / n * (Math.log(class_v[i] / n) / Math.log(2));
		}

		for (int i = 0; i < data.getNumRows(); i++) {
			h_k -= clust_v[i] / n * (Math.log(clust_v[i] / n) / Math.log(2));
		}

		if (h_k == 0.0)
			return h_c;

		return this.getVI() / h_k;
	}
