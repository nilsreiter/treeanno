...
public class TestAlignmentEvaluation {
	TokenAlignmentReader realAReader;

	@Before
	public void setUp() throws Exception {
		realAReader = new TokenAlignmentReader(new File("data2/gold"));
	}

	@Test
	public void testRealEvaluation() throws ParserConfigurationException,
			SAXException, IOException {
		Alignment<Token> gold = realAReader.read(new File(
				"tasks/alignment/gold-3-9-16.xml"));
		Alignment<Token> silver = realAReader.read(new File(
				"tasks/alignment/gold-3-9-16.xml"));
		AlignmentEvaluation<Token> eval = Evaluation
				.getAlignmentEvaluation(Style.REITER);
