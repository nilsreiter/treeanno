...
public class SimilarityConfigurationIterator implements
		Iterator<SimilarityConfiguration> {

	SimilarityConfiguration current;

	List<Field> fields;
	List<ResetableIterator<? extends Object>> iterators;
	boolean inited = false;

	public SimilarityConfigurationIterator(final SimilarityConfiguration sConf) {
		try {
			current = sConf.clone();
	...
	public void setInterestingFields() {
		try {
			...
			iters.add(new DoubleIterator(0.0, 0.9, 0.1));
	...
	public synchronized void addField(final String fieldName,
		...
		this.fields.add(SimilarityConfiguration.class.getField(fieldName));
	...
	public SimilarityConfiguration next(final int i) {
	...
	@Override
	public SimilarityConfiguration next() {
		return next(0);
	}
