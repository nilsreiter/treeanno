...
public class SimilarityConfiguration implements TrainingConfiguration,
	...
	@Option(name = "--sf_arg_idf",
			usage = "Weight tokens in bag of words according to idf value")
	public boolean sf_arg_idf = false;

	@Option(name = "--sf_gaussiandistance_var",
			usage = "This will be used as variance for the gauss curve.")
	public double sf_gaussiandistance_var = 0.2;

	@Option(name = "--sim", aliases = { "-s" }, multiValued = true,
			usage = "The similarity function(s) to use. Can be specified"
					+ " multiple times.")
	public String[] similarityFunctions = null;

	@Option(name = "--weight", aliases = { "-w" }, multiValued = true,
			usage = "The weight(s) for the specified similarity functions."
	...
	@Option(
			name = "--history",
			usage = "If true, the similarity functions try to save their history on disk.")
	...
	public static Object fromString(final String s, final String type) {
		...
		if (type.equals("double")) return new Double(s);
		if (type.startsWith("class")) {
			String className = type.split(" ")[1];
			try {
				Class<?> cl = Class.forName(className);
				if (cl == File.class) return new File(s);
	...
	public String[] getSimilarityFunctions() {
		return similarityFunctions;
	}

	public void setSimilarityFunctions(String[] similarityFunctions) {
