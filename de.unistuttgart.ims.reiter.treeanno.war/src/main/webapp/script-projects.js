...
function init_projects() {
	...
	jQuery.getJSON("rpc/projects", function(data) {
		for (var i = 0; i < data.length; i++) {
			...
			$(tr).append(
					"<td><button class=\"button_open project " + data[i]['id']
							+ "\"></button></td>");
...
function show_exportoptions(projectId, document) {
	...
	area.append("<h2>" + i18n.t("exportoptions.title_for_X", {
		"document" : document['name']
	}) + "</h2>");
	$("#topbar .left").append(
			"<span class=\"adocname\">&nbsp;&gt; "
					+ i18n.t("exportoptions.breadcrumb_for_X", {
						"document" : document['name']
					}) + "</span>");

	area.append("<div class=\"actionbar\"></div>");
	var abar = area.children(".actionbar");
	abar.append("<button class=\"export_xmi\" name=\"export_xmi\" value=\""
			+ document['id'] + "\">" + i18n.t("exportoptions.xmi")
			+ "</button>");
	abar.append("<button class=\"export_par\" name=\"export_par\" value=\""
			+ document['id'] + "\">" + i18n.t("exportoptions.par")
			+ "</button>");
	...
	abar.children(".export_xmi").button({
		...
		text : configuration["treeanno.ui.showTextOnButtons"]
			...
			function(event) {
				window.location.href = "rpc/xmi/" + projectId + "/"
	...
	abar.children(".export_par").button({
		...
		text : configuration["treeanno.ui.showTextOnButtons"]
			...
			function(event) {
				window.location.href = "rpc/par/" + projectId + "/"
						+ event.data.documentId;
			});
...
function show_annodoclist(projectId, id) {
	...
	jQuery.getJSON("rpc/" + projectId + "/" + id, function(data) {
		...
		$(trh).append(
				"<th>" + i18n.t("annodoclistarea.username") + "</th>");
		$(trh).append(
				"<th>" + i18n.t("annodoclistarea.mod_date") + "</th>");
		$(trh).append(
				"<th>" + i18n.t("annodoclistarea.actions") + "</th>");
		$(table).append(trh);
		if (! 'documents' in data)
			data['documents'] = new Array;
		...
		for (var i = 0; i < data['documents'].length; i++) {
			...
			$(tr).append(
					"<td>" + data['documents'][i]['user']['name'] + "</td>");
			$(tr)
					.append(
							"<td>" + data['documents'][i]['modificationDate']
									+ "</td>");

			var actionCell = document.createElement("td");
			$(actionCell).append(
					"<button class=\"button_view\" id=\"view-udoc-"
							+ data['documents'][i]['id']
							+ "\" name=\"view\" value=\""
							+ data['documents'][i]['id'] + "\">"
							+ i18n.t("annodoclistarea.view") + "</button>");
			$(actionCell).append(
					"<button class=\"button_delete\" id=\"delete-udoc-"
							+ data['documents'][i]['id']
							+ "\" name=\"delete\" value=\""
							+ data['documents'][i]['id'] + "\">"
							+ i18n.t("annodoclistarea.delete") + "</button>");
			// if (al >= Perm["PADMINACCESS"])
			$(actionCell).append(
					"<input class=\"button_diff\" id=\"diffselect-"
							+ data['documents'][i]['id']
							+ "\" type=\"checkbox\" name=\"diff\" value=\""
							+ data['documents'][i]['id']
							+ "\"/><label for=\"diffselect-"
							+ data['documents'][i]['id'] + "\"></label>");
			...
			$(actionCell).find("button.button_view").button({
				...
				text : configuration["treeanno.ui.showTextOnButtons"]
					...
					{
						'userDocumentId' : data['documents'][i]['id']
					},
					function(event) {
						window.location.href = "parallel.jsp?userDocumentId="
								+ event.data.userDocumentId;
					});
		...
		$("#annodoclistarea").append(
				"<h2>" + i18n.t("annodoclistarea.title_for_X", {
					"document" : data['src']['name']
				}) + "</h2>");
		$("#annodoclistarea").append(table);
		$("#topbar .left").append(
				"<span class=\"adocname\">&nbsp;&gt; "
						+ i18n.t("annodoclistarea.breadcrumb_for_X", {
							"document" : data['src']['name']
						}) + "</span>");
		$("#annodoclistarea").append(
				"<button id=\"button_open_diff\"></button>");
			...
			text : configuration["treeanno.ui.showTextOnButtons"]
				...
				function() {
					if ($("input.button_diff:checked").length == 2) {
						var doc = new Array();
						$("input.button_diff:checked").each(
								function(index, element) {
									doc[index] = $(element).val();
								});
						window.location.href = "parallel.jsp?userDocumentId="
								+ doc[0] + "&userDocumentId=" + doc[1];
					}
				});

		jQuery.getJSON("rpc/" + projectId + "/u", function(data) {
			var header = false;
	...
	jQuery
			.getJSON(
					...
					function(data) {
						...
						if ("documents" in data) {
							for (var i = 0; i < data['documents'].length; i++) {
								if (!header) {
									var trh = document.createElement("tr");
									$(trh).append(
											"<th>" + i18n.t("document_id")
													+ "</th>");
									$(trh).append(
											"<th>" + i18n.t("document_name")
													+ "</th>");
									// $(trh).append("<th>"+i18n.t("document_mod_date")+"</th>");
									$(trh).append(
											"<th>" + i18n.t("document_actions")
													+ "</th>");
									$(table).append(trh);
									header = true;
								}
								var tr = document.createElement("tr");
								$(tr).append(
										"<td>" + data['documents'][i]['id']
												+ "</td>");
								$(tr).append(
										"<td>" + data['documents'][i]['name']
												+ "</td>");
								...
								if (al >= Perm["READACCESS"])
									$(actionCell)
											.append(
													"<button class=\"button_open\"></button>");
								if (al >= Perm["PADMINACCESS"])
									$(actionCell)
											.append(
													"<button class=\"button_open_master\"></button>");
								if (al >= Perm["PADMINACCESS"])
									$(actionCell)
											.append(
													"<button class=\"button_rename\">rename</button>")
								if (al >= Perm["PADMINACCESS"])
									$(actionCell)
											.append(
								...
								if (al >= Perm["PADMINACCESS"])
									$(actionCell)
											.append(
													"<button class=\"button_view_annodoc\">view annotation</button>");
								if (al >= Perm["PADMINACCESS"])
									$(actionCell)
											.append(
													"<button class=\"button_export\">export</button>");

								$(actionCell)
										.find("button.button_open")
										.button(
												{
													label : i18n
															.t("document_action_open"),
													icons : {
														primary : "ui-icon-document",
														secondary : null
													},
										...
										.click(
												...
												function(event) {
													window.location.href = "main.jsp?documentId="
															+ event.data.documentId;
												});
								$(actionCell)
										.find("button.button_open_master")
										.button(
												{
													label : i18n
															.t("document_action_open_master"),
													icons : {
														primary : "ui-icon-document",
														secondary : null
													},
															...
															.getJSON(
																	...
																	function(
																		...
																		if ('documents' in data
																			...
																			if (confirm(i18n
																					.t("document_action_open_master_confirm"))) {
																				window.location.href = "main.jsp?master=master&documentId="
																						+ event.data.documentId;
																			}
																		} else {
																			window.location.href = "main.jsp?master=master&documentId="
								...
								$(actionCell)
										.find("button.button_rename")
										.button(
												{
													label : i18n
															.t("document_action_rename"),
													icons : {
										...
										.click(
												...
												function(event) {
													var diagDiv = document
															.createElement("div");
													$(diagDiv)
															.append(
																	i18n
																			.t("rename_dialog.desc")
								...
								$(actionCell)
										.find("button.button_delete")
										.button(
												{
													label : i18n
															.t("document_action_delete"),
													icons : {
														primary : "ui-icon-trash",
														secondary : null
													},
										...
										.click(
												...
												function(event) {
													if (confirm(i18n
															.t("document_action_delete_confirm"))) {
														jQuery
																.ajax({
																	url : "rpc/c/"
																			+ id
								...
								$(actionCell)
										.find("button.button_view_annodoc")
										.button(
												{
													label : i18n
															.t("document_action_view_annodoc"),
													icons : {
								...
								$(actionCell)
										.find("button.button_export")
										.button(
												{
													label : i18n
															.t("document_action_export_xmi"),
													icons : {
						...
						$("#documentlistarea").append(
								"<h2>" + i18n.t("documents_in_X", {
						...
						if (al >= Perm["PADMINACCESS"]) {
							$("#documentuploaddialog input[name='projectId']")
									.attr("value", data['project']['id']);
							$("#documentlistarea")
									.append(
											"<button data-i18n=\"new_document.open_dialog\" id=\"new_document_open_dialog\"></button>");
							$("button#new_document_open_dialog")
									.button(
											{
												label : i18n
														.t("new_document.open_dialog"),
												icons : {
...
function select_document_for_diff(did) {
	if ($("#diffselect-" + did).val() == did) {
		...
		documents_selected_for_diff.splice(documents_selected_for_diff
				.indexOf(did), 1);
