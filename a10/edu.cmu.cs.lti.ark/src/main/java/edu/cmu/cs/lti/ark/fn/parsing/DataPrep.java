...
public class DataPrep {
	...
	/**
	 * @brief an array list containing candidate spans for each sentence the
	 *        m'th span in the n'th sentence is canLines.get(n)[m][0] and
	 *        canLines.get(n)[m][1]
	 */
	public ArrayList<int[][]> canLines;
	/**
	 * a hash map from a frame name to its frame elements.
	 */
	...
	/**
	 * prints .spans file , which is later used to recover frame parse after
	 * prediction
	 */
	...
	public DataPrep(final String spansFile) {
		ps = FileUtil.openOutFile(FEFileName.spanfilename);
		load(null, null, null);
	}

	public DataPrep(final ArrayList<String> tL, final ArrayList<String> fL,
			final WordNetRelations lwnr) {
		// this model does not write span files
		ps = FileUtil.openOutFile(FEFileName.spanfilename);
		load(tL, fL, lwnr);
	}

	private ArrayList<String> readLinesInFile(final String filename) {
	...
	private ArrayList<int[]> addConstituent(final DataPointWithElements dp) {
					...
					for (int k = 1; k < nodes.length; k++) {
						...
						if (FEFileName.unlabeledSpans.contains(span)) {
							if (!spanMat[j - 1][k - 1]) {
								...
								depParses[j - 1][k - 1] = findParse(
										dp.getParses(), j, k,
										FEFileName.unlabeledSpans.get(span));
							}
						}
					}
				}
	...
	public int findParse(final DependencyParses parses, final int j,
			final int k, int hIndex) {
		hIndex = hIndex + j;
		int start = 0;
		int ret = 0;
		Range1Based span = new Range1Based(j, k);
		while (true) {
			Pair<Integer, DependencyParse> p = parses.matchesSomeConstituent(
					span, start);
			if (p == null) {
				ret = 0;
				break;
	 ...
	 * 
	 */
	private void load(final ArrayList<String> tL, final ArrayList<String> fL,
			final WordNetRelations lwnr) {
		if (fedict == null) {
		...
		for (String feline : feLines) {
			...
			DataPointWithElements dp = new DataPointWithElements(
					tagLines.get(sentNum), feline);
			int spans[][];
	...
	public void addGoldSpan(final boolean[][] spanMat,
			final DataPointWithElements goldDP, final int[][] depParses) {
		...
		for (Range0Based span : spans) {
			...
			if (FEFileName.KBestParse > 1) {
				...
				Pair<Integer, DependencyParse> p = parses
						.matchesSomeConstituent(span1, start);
				if (p == null)
	...
	public static void addFeature(final String key,
			final HashMap<String, Integer> freqmap) {
	...
	public int[][][] getTrainData() {
		...
		int sentNum = Integer.parseInt(feline.split("\t")[5]);
		DataPointWithElements goldDP = new DataPointWithElements(
				tagLines.get(sentNum), feline);
		...
		for (int i = 0; i < goldDP.getNumOvertFrameElementFillers(); i++) {
			...
			addFeatureForOneArgument(goldDP, frame, fename[i], spans.get(i),
	...
	public int[][][] getTrainDataOracleSpans() {
		...
		for (int i = 0; i < goldDP.getNumOvertFrameElementFillers(); i++) {
			realizedFes.add(fename[i]);
			addFeatureForOneArgumentOracleSpans(goldDP, frame, fename[i],
					spans.get(i), datapointList, cantoks);
		}
		// add null frame elements
		if (canArgs != null)
			for (String fe : canArgs) {
	...
	private void addFeatureForOneArgumentOracleSpans(
			final DataPointWithElements goldDP, final String frame,
			final String fe, final Range0Based span,
			final ArrayList<int[][]> datapointlist, final int cantoks[][]) {
		...
		for (int j = 0; j < cantoks.length; j++) {
			...
			Range0Based canspan;
			canspan = CandidateFrameElementFilters.createSpanRange(
					cantoks[j][0], cantoks[j][1]);
	...
	private void addFeatureForOneArgument(final DataPointWithElements goldDP,
			final String frame, final String fe, final Range0Based span,
			final ArrayList<int[][]> datapointlist, final int cantoks[][]) {
		Set<String> featureSet;
		ArrayList<int[]> adatapoint = new ArrayList<int[]>();
		// add feature for gold standard
	...
	public static void readFeatureIndex(final String alphabetFilename) {
	...
	public static void writeFeatureIndex(final String alphabetFilename) {
	...
	public static void findSpans(final boolean[][] spanMat,
			final DependencyParse[] nodes) {
	...
	public static void findSpans(final boolean[][] spanMat,
			final int[][] heads, final DependencyParse[] nodes) {
		int[] parent = new int[nodes.length - 1];
		int left[] = new int[parent.length];
		int right[] = new int[parent.length];
		for (int i = 0; i < parent.length; i++) {
		...
		for (int i = 0; i < parent.length; i++) {
			...
			if (i - 1 >= 0) {
				if (spanMat[left[i]][justLeft]) {
					if (justLeft - left[i] == 0
							&& nodes[justLeft + 1].getPOS().equals("DT")) {
						;
					} else if (justLeft - left[i] == 0
							&& nodes[justLeft + 1].getPOS().equals("JJ")) {
						;
					} else {
	...
	public int featidx(final String feature) {
