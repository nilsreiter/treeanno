...
public class AlphabetCreationThreaded {
	...
	public static void main(final String[] args) {
		...
		AlphabetCreationThreaded events = new AlphabetCreationThreaded(
	...
	public AlphabetCreationThreaded(final String alphabetFile,
	...
	public void processBatch(final int i) {
		mLogger.info("Thread " + i + ": Creating events....");
		int dataCount = mEndIndex - mStartIndex;
		int batchSize = (int) (Math.ceil((double) dataCount
				/ (double) mNumThreads));
		...
		// System.out.println("Thread " + i + ": start:" + start +" end:" +
		// end);
		int count = 0;
		...
		try {
			...
			while ((line = bReader.readLine()) != null) {
				...
				Pair<String, Integer> pair = processLine(line, count,
						parseLine, parseOffset, parseReader, alphabet, i);
				count++;
				if (count == end) {
					break;
				}
			...
			System.err.println("Problem in Thread:" + i);
			System.err.println("Problem count:" + count);
			e.printStackTrace();
			System.exit(0);
		}
	...
	private void writeAlphabetFile(final Map<String, Integer> alphabet,
			final String file) {
		try {
	...
	private int[][] getFeatures(final String frame, final int[] intTokNums,
			final String[][] data, final Map<String, Integer> alphabet) {
	...
	private Pair<String, Integer> processLine(final String line,
			...
			final Map<String, Integer> alphabet, final int threadIndex) {
		...
		mLogger.info("Thread:" + threadIndex + " Index:" + index + " feLine:"
				+ line);
		mLogger.info("Thread:" + threadIndex + " Index:" + index + " feLine:"
				+ sentNum);
		mLogger.info("Thread:" + threadIndex + " Index:" + index
		...
		// mLogger.info("Thread:" + threadIndex + " Index:"+index
		// +" Number of toks:" + tokensInFirstSent);
