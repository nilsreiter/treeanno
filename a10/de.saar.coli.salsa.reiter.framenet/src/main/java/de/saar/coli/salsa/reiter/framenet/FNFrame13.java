...
/**
 * This class implements frames based on the FrameNet version 1.3 and 1.4-alpha.
 * 
 * @author reiter
 * @since 0.4.2
 * 
 */
public class FNFrame13 extends FNFrame {
    ...
    /**
     * Creates a new FNFrame13 object. Should not be called directly.
     * 
     * @param node
     ...
     *            The database reader
     */
    protected FNFrame13(final Element node, final FNDatabaseReader reader) {
	...
	List<?> lexunitNodelist = node.element("lexunits").elements("lexunit");
	for (int i = 0; i < lexunitNodelist.size(); i++) {
	    Element lu = (Element) lexunitNodelist.get(i);

	    LexicalUnit luo = new FNLexicalUnit13(reader, this, lu);
	...
	List<?> feNodelist = node.element("fes").elements("fe");
	for (int i = 0; i < feNodelist.size(); i++) {
	    Element fe = (Element) feNodelist.get(i);
	    frameElements.put(fe.attributeValue("name"), new FNFrameElement13(
		    reader, this, fe));
	}
