...
public class TestAlignmentEvaluationBlanc {
	Token[] t;
	Document[] doc;

	@Before
	public void setUp() throws Exception {
		doc = new Document[] { mock(Document.class), mock(Document.class) };
		t = new Token[] { mock(Token.class), mock(Token.class),
				mock(Token.class), mock(Token.class), mock(Token.class),
				mock(Token.class) };
		for (int i = 0; i < t.length; i++) {
			when(t[i].getRitualDocument()).thenReturn(doc[i % 2]);
		}
	}
	...
	public void testEverythingAligned() {
		...
		silver.addAlignment("al0", Arrays.asList(t));

		AlignmentEvaluation<Token> eval = Evaluation
				.getAlignmentEvaluation(Style.BLANC);

		SingleResult res = eval.evaluate(gold, silver);
		assertEquals(0.5, res.getScore(Evaluation.RECALL), 0.0);
		assertEquals(1.0, res.getScore(Evaluation.PRECISION), 0.0);
		assertEquals(1.0, res.getScore(Evaluation.FSCORE), 1e-3);
