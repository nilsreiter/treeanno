...
public class JiangAndConrath {
	...
	/*
	 * (cribbed from Ted's site) This module computes the semantic relatedness
	 * of word senses according to the method described by Jiang and Conrath
	 * (1997). This measure is based on a combination of using edge counts in
	 * the WordNet 'is-a' hierarchy and using the information content values of
	 * the WordNet concepts, as described in the paper by Jiang and Conrath.
	 * Their measure, however, computes values that indicate the semantic
	 * distance between words (as opposed to their semantic relatedness). In
	 * this implementation of the measure we invert the value so as to obtain a
	 * measure of semantic relatedness. Other issues that arise due to this
	 * inversion (such as handling of zero values in the denominator) have been
	 * taken care of as special cases.
	 * 
	 * The relatedness value returned by the jcn measure is equal to 1 /
	 * jcn_distance, where jcn_distance is equal to IC(synset1) + IC(synset2) -
	 * 2 * IC(lcs). The original metric proposed by Jiang and Conrath was this
	 * distance measure. By taking the multiplicative inverse of it, we have
	 * converted it to a measure of similarity, but by so doing, we have shifted
	...
	public double jcn(String w1, int s1, String w2, int s2, String pos) {
		...
		// 2.
		if ((synset1.equals(synset2) && synset2.equals(lcs))
				|| ((ic1 + ic2) == (2.0 * ic3))) {
			double rootsum = icfinder.getRootSum(pos);
			jcn = (1.0 / (-Math.log((rootsum - 0.01) / rootsum))); // !!! as per
																	// Perl
																	// version...
																	// weird-a-rama!!!
			return (jcn);
		 ...
		 * computing relatedness; both of these involve the case when
		 * jcn_distance is zero. //In the first case, we have ic(synset1) =
		 * ic(synset2) = ic(lcs) = 0. In an ideal world, this would only happen
		 * when all three concepts, viz. synset1, synset2, and lcs, are the
		 * //root node. However, when a synset has a frequency count of zero, we
		 * use the value 0 for the information content. In this first case, we
		 * return 0 due to lack of data.
		 * 
		 * //In the second case, we have ic(synset1) + ic(synset2) = 2 *
		 * ic(lics). This is almost always found when synset1 = synset2 = lcs
		 * (i.e., the two input synsets are the same). //Intuitively this is the
		 * case of maximum relatedness, which would be infinity, but it is
		 * impossible to return infinity. Insteady we find the smallest possible
		 * distance greater than //zero and return the multiplicative inverse of
		 * that distance.
		 */
		return (jcn);
	}
		...
		if (word1 != null && word2 != null) {
			...
			for (IWordID idX : word1IDs) {
				...
				for (IWordID idY : word2IDs) {
					...
					map.put((word1.getLemma() + "#" + pos + "#" + sx + ","
							+ word2.getLemma() + "#" + pos + "#" + sy),
							jcnscore);
					sy++;
				}
				sx++;
	...
	public TreeMap<String, Double> jcn(String w1, String w2, int s2, String pos) {
		...
		if (word1 != null && word2 != null) {
			...
			System.out.println(word1.getLemma() + " and/or " + word2.getLemma()
					+ " in POS " + pos + " do not exist in WordNet "
					+ dict.getVersion());
			return (map);
		}
		return (map);
	...
	public TreeMap<String, Double> jcn(String w1, int s1, String w2, String pos) {
		...
		if (word1 != null && word2 != null) {
			...
			for (IWordID idX : word2IDs) {
				...
				map.put((word1.getLemma() + "#" + pos + "#" + s1 + ","
						+ word2.getLemma() + "#" + pos + "#" + movingsense),
						jcnscore);
				movingsense++;
			}
		} else {
	...
	public ISynset getLCS(ISynset synset1, ISynset synset2, String pos) {
		...
		for (ISynsetID h : h1) {
			double ic = icfinder.getIC("" + h.getOffset(), pos); // use ICfinder
																	// to get
																	// the
																	// Information
																	// Content
		...
		return (dict.getSynset(maxlcs)); // return the <synset} with *a* highest
											// IC value
	}
	...
	public static void main(String[] args) throws IOException {
		...
		/*
		 * double jcnscore = jcn.jcn("contrition8", 1, "compunction", 1, "n");
		 * // "word1", sense#, "word2", sense#, "POS"
		 * System.out.println("specific senses"); if(jcnscore != 0) // 0 is an
		 * error code i.e it means that something isn't right e.g. words are not
		 * in WordNet, wrong POS etc { System.out.println("jcn:\t" +
		 * formatter.format(jcnscore)); } System.out.println();
		 */

		// jcn(2) all senses: a value (score) of 0 is an error code for a pair
		TreeMap<String, Double> map = jcn.jcn("cat", "dog", "n"); // "word1",
																	// "word2",
																	// "POS"
		 ...
		 * 
		 * // jcn(4) a specific sense of word 1 vs. all senses of word 2: a
		 * value (score) of 0 is an error code for a pair TreeMap<String,Double>
		 * map4 = jcn.jcn("eat8", 1, "consume", "v"); // "word1", sense#,
		 * "word2", "POS"
