...
/**
 * This class implements a sequence of characters stored as an array that
 * provides random access to any position in constant time.
 *
 * <P>
 * The input can come from any source, provided it is encapsulated in a proper
 * <CODE>Reader</CODE> instance. The stream is expected to be ready (i.e. the
 * next <CODE>read</CODE> operation must return the first character of the
 * sequence) and it is not closed when its end is reached, so the client is
 * allowed to reset it and maybe use it for another purpose.
 * </P>
 *
 * <P>
 * Sequences can contain letters only although lines started with the
 * <CODE>COMMENT_CHAR</CODE> character ('>') are regarded as comments and are
 * completely skipped. White spaces (including tabs, line feeds and carriage
 * returns) are also ignored throughout.
 * </P>
 *
 * <P>
 * This class is used by two sequence alignment algorithms:
 * {@linkplain SmithWaterman} and {@linkplain NeedlemanWunsch}.
 * </P>
 *
...
public class CharSequence {
	/**
	 * The character used to start a comment line in a sequence file. When this
	 * character is found, the rest of the line is ignored.
	 */
	 ...
	 */
	public CharSequence(Reader reader) throws IOException,
