...
public class NeedlemanWunsch<T> extends PairwiseAlignmentAlgorithm<T> {
	...
	public NeedlemanWunsch(final ScoringScheme<T> scoring) {
	...
	protected PairwiseAlignment<T> buildOptimalAlignment()
		...
		while ((r > 0) || (c > 0)) {
			...
			if ((r > 0) && (c > 0)) {
				sub = (int) scoreSubstitution(seq1.get(r - 1), seq2.get(c - 1));
				...
				if (matrix[r][c] == matrix[r - 1][c - 1] + sub) {
					...
					if (seq1.get(r - 1).equals(seq2.get(c - 1)))
						score_tag_line.add(0, new IndividualAlignment(
								AlignmentType.Full, 1.0));
					else if (sub > 0.0)
						score_tag_line.add(0, new IndividualAlignment(
								AlignmentType.Partial, 0.5));
					else
						score_tag_line.add(0, new IndividualAlignment(
								AlignmentType.None, 0.0));
					gapped_seq2.add(0, seq2.get(c - 1));
					r = r - 1;
					c = c - 1;
	...
	protected int computeScore() throws IncompatibleScoringSchemeException {
		...
		if (rows <= cols) {
			...
			array[0] = 0;
			for (r = 1; r < rows; r++)
				array[r] = array[r - 1] + (int) scoreDeletion(seq1.get(r));
			...
			for (c = 1; c < cols; c++) {
				...
				tmp = array[0] + (int) scoreInsertion(seq2.get(c));
